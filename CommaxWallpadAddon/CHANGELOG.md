...
### 변경됨
- 코드베이스 정리 및 기능 개선
  - 더 이상 사용되지 않는 main_prev.py 파일 제거
  - script.js 파일의 구조 및 가독성 개선
    - 기기 패킷 테이블 구조 개선
    - 기기별 패킷 테이블 동적 생성 기능 강화
  - 패킷 정보 표시를 위한 HTML 템플릿 및 CSS 스타일 개선

## [1.4.4] - 2024-12-24

### 개선됨
- 기기 패킷 구조 개선 및 HTML 템플릿 업데이트
  - devices_and_packets_structures.yaml에 'ack' 구조 추가
  - Gas 및 EV 기기 값 업데이트로 명령 매핑 정확성 향상
  - index.html에서 불필요한 인라인 CSS 제거 및 외부 스타일시트 연결
  - 패킷 로그 섹션 개선으로 감지된 패킷 정보 정확성 향상
...
### 개선됨
- WallpadController의 기 상태 처리 로직 개선
